...
def test_compaction_aux_model_call_gets_own_governed_request(tmp_path, monkeypatch):
    ...
    dispatched_requests = [
        state for state in budget["request_state"].values()
        if int(state.get("dispatch_count", 0) or 0) == 1
    ]
    assert sum(int(state.get("dispatch_count", 0) or 0) for state in dispatched_requests) == len(dispatched_requests)
    assert budget["consumed"]["model_calls"] == sum(
        int(state.get("reconciliation", {}).get("actual_model_calls", 0) or 0)
        for state in dispatched_requests
    )
    assert budget["consumed"]["model_calls"] <= 12
    assert all(state.get("request_kind") in {"primary", "compaction"} for state in dispatched_requests)
