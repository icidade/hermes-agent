...
class GovernanceController:
    ...
    def resolve_artifact_reference(self, reference: str) -> Path:
        """Resolve a portable engagement-relative artifact reference safely."""
        candidate = Path(str(reference))
        if candidate.is_absolute():
            raise ValueError("artifact references must be relative")
        return self._validate_engagement_path(self._engagement_dir() / candidate, self._engagement_dir())
        ...
        *,
        ...
        if not payload["allowed"] and action in {"approval", "hard_stop"}:
            ...
            self.finalize_request(
                request_id,
                "blocked",
                termination_reason="authorization_required" if action == "approval" else "cost_context_governance_hard_stop",
            )
            summary = (
    ...
    def persist_partial_handoff(
        ...
        *,
        ...
        artifact_path = f"partial_handoffs/{self.task_id}-{int(time.time())}.json"
        payload = {
            "status": status,
            "task": task,
            ...
            "artifact_references": [*(artifact_references or []), artifact_path],
        ...
        path = self._persist_json(artifact_path, project_artifact_value(payload))
